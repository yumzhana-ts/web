...
/*void CgiHandler::runParent(const char* input) 
{
    ...
    while (!stdinClosed || !stdoutClosed) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if ((events[i].events & EPOLLOUT) && !stdinClosed) {
                ...
                if (written >= len) {
                    ...
                    stdinClosed = true;
                    epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, NULL); // stop monitoring
                }
            }

            // --- Read output from CGI
            if ((events[i].events & EPOLLIN) && !stdoutClosed) {
                ...
                if (r > 0) {
                    ...
                    std::cout << "Total size received: " << total_received << " bytes" << std::endl;
                }
            }
        }
    }
...
void CgiHandler::runParent(const char* input) {
    ...
    std::stringstream cgi_output;
    char buffer[64*1024]; // read in chunks

    while (!stdinClosed || !stdoutClosed) {
        struct epoll_event events[2];
        int n = epoll_wait(epoll_fd, events, 2, 1000); // 1s timeout
        if (n < 0) {
        ...
        // check child exit if stdin is closed but stdout not done
        if (stdinClosed && !stdoutClosed) {
            int status;
            pid_t w = waitpid(pid, &status, WNOHANG);
            if (w == pid) {
                std::cerr << "child exited, closing stdout" << std::endl;
                close(pipe_out[0]);
                stdoutClosed = true;
                raw_cgi_response = cgi_output.str();
                break;
        ...
        for (int i = 0; i < n; ++i) {
            ...
            // --- Write to stdin
            if (!stdinClosed && fd == pipe_in[1] && (events[i].events & EPOLLOUT)) {
                while (written < len) {
                    ssize_t w = write(fd, input + written, len - written);
                    if (w > 0) {
                        written += w;
                        std::cerr << "wrote " << w << " bytes, total=" << written << "/" << len << std::endl;
                    } else if (w == -1 && errno == EAGAIN) {
                        break; // pipe full, retry later
                    } else {
                        perror("write failed");
                        break;
                    }
                }
                if (written >= len && !stdinClosed) {
                    close(fd);
                    stdinClosed = true;
                    epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, NULL);
                    std::cerr << "stdin closed after writing all bytes" << std::endl;
                }
            }

            // --- Read from stdout
            if (!stdoutClosed && fd == pipe_out[0] && (events[i].events & EPOLLIN)) {
                while (true) {
                    ssize_t r = read(fd, buffer, sizeof(buffer));
                    if (r > 0) {
                        cgi_output.write(buffer, r);
                        std::cerr << "read " << r << " bytes from stdout" << std::endl;
                    } else if (r == 0) {
                        close(fd);
                        stdoutClosed = true;
                        epoll_ctl(epoll_fd, EPOLL_CTL_DEL, fd, NULL);
                        raw_cgi_response = cgi_output.str();
                        std::cerr << "stdout closed by EOF" << std::endl;
                        break;
    ...
    Logger::debug(" 🩸 [CGI Handler] Parent finished [Total written = " + toString(written) + "] [Total output=" + toString(raw_cgi_response.size()) + "]");
}
